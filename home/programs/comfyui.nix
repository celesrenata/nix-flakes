...
let
  ...
  comfyui-wrapper = pkgs.writeShellScript "comfyui-wrapper" ''
    ...
    if [ ! -f ${config.home.homeDirectory}/.config/comfy-ui/.deps_complete ]; then
      ...
      $VENV_DIR/bin/pip install pyyaml segment-anything dill facexlib piexif insightface deepdiff webcolors ultralytics py-cpuinfo gguf llama-cpp-python onnxruntime imageio-ffmpeg opencv-python numba pynvml timm
