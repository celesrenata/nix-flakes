...
in
{
  mcp-server-sequential-thinking = buildMcpServer {
    ...
    depsHash = if final.stdenv.hostPlatform.isAarch64
      then "sha256-2c+snmuNQeHNOKHBIBOLGCjjrFSaZiGh3LZtIMp9+1A="
  ...
  mcp-server-memory = buildMcpServer {
    ...
    depsHash = if final.stdenv.hostPlatform.isAarch64
      then "sha256-n9JJPa8190cL78j0Sx5HZD7MpfazXm2Lue9s3XD7UNO="
  ...
  mcp-server-github = buildMcpServer {
    ...
    depsHash = if final.stdenv.hostPlatform.isAarch64
      then "sha256-XBmhiAnZBs4NorY7UTzKbciS4bYQSuHuO2ATm+t+oms="
  ...
  any-chat-completions-mcp = buildMcpServer {
    ...
    depsHash = if final.stdenv.hostPlatform.isAarch64
      then "sha256-SPq1I75BYi0e5q5tFFvWCiJ1yG7GRnUM0WOjPUxs8/M="
